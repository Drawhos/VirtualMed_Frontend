...
export function RagDocumentsAdminView() {
  ...
  const canUpload = useMemo(
    () => (user?.permission ?? []).includes('RagDocument:Upload'),
    [user?.permission]
  );
  const canDelete = useMemo(
    () => (user?.permission ?? []).includes('RagDocument:Delete'),
    [user?.permission]
  );
    ...
    const uploadNext = async () => {
      ...
      try {
        ...
        setQueue((prev) =>
          prev.map((item) =>
            item.id === itemId
              ? { ...item, status: 'done', message: 'Indexado correctamente' }
              : item
          )
        );
        await loadDocuments();
        ...
        setQueue((prev) =>
          prev.map((item) =>
            item.id === itemId ? { ...item, status: 'error', message } : item
          )
        );
      } finally {
  ...
  const handleDelete = async (document: RagDocument) => {
    ...
    try {
      await ragDocumentService.delete(document.id);
      await loadDocuments();
    } catch (error) {
      console.error(error);
    ...
    <div className="mt-14 space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-600">
          Sube PDFs clínicos para el asistente. Máximo 20 MB por archivo. Los archivos se indexan uno a la vez.
        </p>
      </div>
